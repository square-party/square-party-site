...
## Deploy architecture (corrected 2026-05-13)

**Canonical reference: `DEPLOY.md` at this repo root.** Open that first for the actual deploy commands and architecture.

Quick summary (overrides any earlier claims in this file):

- **`square-party-site` Cloudflare Pages project** — bound to `squareparty.org`. Auto-builds from GitHub on push (Pages git integration runs Eleventy server-side). Serves all static content.
- **`square-generator` Cloudflare Worker** — bound to `squareparty.org/api/*` via Worker Route in `square-generator-backend/worker/wrangler.toml`. Serves square submission, gallery API, admin moderation. Has D1 binding + Turnstile secret. Deploys via `npm run deploy` (repurposed 2026-05-13 to deploy this Worker) or directly: `cd square-generator-backend/worker && npx wrangler deploy -c wrangler.toml`.

Routing precedence: Worker Routes (e.g. `squareparty.org/api/*` → `square-generator`) take priority over Pages custom-domain bindings. Pages serves everything else; the two coexist cleanly.

`/admin/*` (HTML page, served by Pages) AND `/api/admin/*` (API endpoint, on the Worker) are both gated by ONE Cloudflare Access application. The Access app must include both paths in its configuration; missing the API path leaves the admin queue page broken with "unauthorized" because the JS-side fetch to `/api/admin/queue` reaches the Worker without the `Cf-Access-Authenticated-User-Email` header.

The old `square-party` Worker (per root `wrangler.jsonc`) was unbound and unused; deleted from Cloudflare 2026-05-13. Root `wrangler.jsonc` lives in the repo as legacy but isn't deployed by the current `npm run deploy`.
...
- **Paul does**: full builds (`npm run build`) for local preview, GitHub commits (Pages auto-deploys on push), Wrangler deploys via `npm run deploy` when the API Worker needs updating. See `DEPLOY.md` for the canonical reference.
